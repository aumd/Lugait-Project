...
class ReportController {
	...
	def printReport(){
		...
		def result = db.rows("SELECT resident.purok, resident.precinct_number, resident.resident_number, resident.resident_child_name, resident.resident_name, resident.birth_date, resident.birth_month, resident.birth_year, resident.address, resident.age, resident.educational_attainment, resident.employment_status, resident.household_role, resident.monthly_income, resident.med_history, resident.environmental_sanitation, resident.means_of_drinking_water, resident.means_of_living, resident.disabilities,resident.status, resident.religion, resident.resident_spouse_name  FROM resident where resident.id = '${residentInstance.id}' order by resident.id")
